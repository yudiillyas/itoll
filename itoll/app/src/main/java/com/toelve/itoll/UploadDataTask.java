...
public class UploadDataTask extends AsyncTask<String,Void,String> {
   ...
   private  AlertDialog alertDialog;
    ...
    protected String doInBackground(String... params) {
        String login_url = "http://toelve.com/itoll/mobile/driver/masuk";
        String method = params[0];
        if (method.equalsIgnoreCase("upload")) {
            username = params[1];
            String namauser = params[2];
            String pintu = params[3];
            String noplat = params[4];
            String status = params[5];
            String namapintu = params[6];
            String jam = params[7];
            String tanggal = params[8];
            String nomerhp = params[9];
            String harga = params[10];
            String point = params[11];
            ...
            try {
                ...
                String data = URLEncoder.encode("username", "UTF-8") + "=" + URLEncoder.encode(username, "UTF-8") + "&" +
                        URLEncoder.encode("namauser", "UTF-8") + "=" + URLEncoder.encode(namauser, "UTF-8") + "&" +
                        URLEncoder.encode("pintu", "UTF-8") + "=" + URLEncoder.encode(pintu, "UTF-8") + "&" +
    ...
    protected void onPostExecute(String result) {
        try {
            ...
            if (result.trim().equalsIgnoreCase("Sukses")) {
                     ctx.startActivity(new Intent(ctx,Tunggukeluar.class));
                ((Activity)ctx).finish();
            } else {
